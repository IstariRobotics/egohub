...
def main():
    parser = argparse.ArgumentParser(
        description="Train a Multimodal VAE on egocentric data."
    )
    ...
    sample = dataset[0]
    pose_input_dim = (
        sample["skeleton_positions"].flatten().shape[0]
    )  # Add hand poses if used
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_data in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{args.num_epochs}"):
            ...
            batch_data["skeleton_positions"] = batch_data["skeleton_positions"].to(
                device
            )
