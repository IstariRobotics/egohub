...
def main():
    ...
    parser.add_argument("h5_path", type=Path, help="Path to the HDF5 file with latent vectors.")
    parser.add_argument("--sequence-length", type=int, default=16, help="Sequence length for training.")
    ...
    parser.add_argument("--d-hid", type=int, default=256, help="Dimension of the feedforward network in the Transformer.")
    parser.add_argument("--nlayers", type=int, default=3, help="Number of Transformer encoder layers.")
    ...
    parser.add_argument("--output-path", type=Path, default=Path("policy_checkpoint.pth"), help="Path to save the trained model checkpoint.")

    args = parser.parse_args()
    device = torch.device(args.device)
    
    # 1. Dataset and DataLoader
    dataset = LatentSequenceDataset(args.h5_path, sequence_length=args.sequence_length + 1) # +1 for target
