...
def main():
    parser = argparse.ArgumentParser(description="Encode a dataset using a trained Multimodal VAE.")
    parser.add_argument("h5_path", type=Path, help="Path to the canonical HDF5 file to encode.")
    parser.add_argument("checkpoint_path", type=Path, help="Path to the trained VAE model checkpoint.")
    parser.add_argument("--latent-dim", type=int, default=128, help="Dimensionality of the latent space (must match trained model).")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size for encoding.")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu", help="Device for encoding.")
    ...
    with h5py.File(args.h5_path, 'a') as f:
        for traj_name, latents in tqdm(trajectory_latents.items(), desc="Writing to HDF5"):
            if traj_name in f:
                ...
                print(f"Warning: Trajectory '{traj_name}' not found in HDF5 file. Skipping.")
