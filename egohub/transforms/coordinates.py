...
def arkit_to_canonical_transform() -> np.ndarray:
    ...
    """
    transform = R.from_euler('x', 90, degrees=True).as_matrix()
...
def transform_pose(pose: np.ndarray, transform_matrix: np.ndarray) -> np.ndarray:
    ...
    if transform_matrix.shape != (4, 4):
        raise ValueError(f"Expected 4x4 transform matrix, got shape {transform_matrix.shape}")
        
    return transform_matrix @ pose


def transform_poses_batch(poses: np.ndarray, transform_matrix: np.ndarray) -> np.ndarray:
    """
...
def arkit_to_canonical_poses(poses: np.ndarray) -> np.ndarray:
    ...
    Returns:
        A numpy array with the same shape, with poses in the canonical coordinate system.
    """
