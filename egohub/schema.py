...
def validate_hdf5_with_schema(h5_group: h5py.Group, schema_cls: Type[Any] | Any, path: str = "", strict: bool = False):
    """
        ...
        # Validate datasets
            ...
            if expected_dtype:
                if f.type == h5py.special_dtype(vlen=bytes):
                    ...
                    msg = f"Validation: Mismatched dtype for dataset '{current_path}'. Got {member.dtype}, expected {expected_dtype}"
                    if strict:
                        raise SchemaValidationError(msg)
                    logging.warning(msg)

            if expected_shape_meta and len(member.shape) != len(expected_shape_meta):
                # We only validate rank, not specific dimension sizes (which can be variable, e.g., -1)
                msg = f"Validation: Mismatched rank for dataset '{current_path}'. Got rank {len(member.shape)}, expected rank {len(expected_shape_meta)}"
                if strict:
                    raise SchemaValidationError(msg)
                logging.warning(msg)
        ...
        if get_origin(f.type) is dict:
            ...
            for child_name, child_group in dict_group.items():
                if isinstance(child_group, h5py.Group):
                    validate_hdf5_with_schema(child_group, value_type, f"{path}/{f.name}/{child_name}", strict=strict)
