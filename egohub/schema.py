    ...
    "trajectory_{:04d}": {
        ...
        "metadata": {
            ...
            "attributes": {
                ...
                "action_label": (
                    "A short, high-level description of the action performed."
                ),
            },
            "datasets": {
                "timestamps_ns": (
                    "Synchronized timestamps for all data streams (uint64, in "
                    "nanoseconds)."
                ),
            },
        },
        "cameras": {
            ...
            "{camera_name}": {
                ...
                "datasets": {
                    "pose_in_world": (
                        "4x4 homogenous transformation matrix (world <- camera)."
                    ),
                ...
                "rgb": {
                    ...
                    "datasets": {
                        "image_bytes": (
                            "Variable-length dataset of JPG-compressed image bytes."
                        ),
                ...
                "depth": {
                    ...
                    "attributes": {
                        "scale_factor": (
                            "Multiplier to convert depth values to meters "
                            "(e.g., 1000.0 for 16-bit millimeter data)."
                        )
                    },
                    "datasets": {
        ...
        "hands": {
            ...
            "left": {
                ...
                "datasets": {
                    "pose_in_world": (
                        "4x4 homogenous transformation matrix for the wrist "
                        "(world <- hand)."
                    ),
                    "mano_betas": "10 MANO shape parameters.",
                    "mano_thetas": (
                        "48 MANO pose parameters (3 global rotation + "
                        "45 joint rotations)."
                    ),
        ...
        "objects": {
            ...
            "object_{:02d}": {
                ...
                "attributes": {
                    "label": (
                        "Semantic label for the object (e.g., 'cup', 'screwdriver')."
                    )
                },
                "datasets": {
                    "pose_in_world": (
                        "4x4 homogenous transformation matrix (world <- object)."
                    ),
