...
class EgoDexAdapter(BaseAdapter):
    ...
    def process_sequence(self, seq_info: dict, traj_group: h5py.Group):
        ...
        print("="*80)
        # Populate the template with the camera name for validation
        validation_streams = [
            s.replace('{camera_name}', 'ego_camera') for s in CANONICAL_DATA_STREAMS_TEMPLATE
        ]
        for stream in sorted(list(validation_streams)):
            # We don't expect object or depth data from egodex
            if 'object' in stream or 'depth' in stream:
                continue
